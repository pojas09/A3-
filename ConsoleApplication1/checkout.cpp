...
Checkout::Checkout(const string& dataEntrada, const string& dataSaida, int dias, Cliente* cliente, Carro* carro, const float& precoFinal, const float& precoDano, const float& precoMulta, const float& precoGasolina) : Checkin (dataEntrada, dataSaida, dias, carro, cliente), precoFinal(precoFinal), danoPreco(precoDano), multasPreco(precoMulta), gasolinaPreco(precoGasolina) {}
